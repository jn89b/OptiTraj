    ...
    def compute_cost(self, obstacle_x, obstacle_y):
        ...
        for k in range(N):
            ...
            state_next_RK4 = states + \
                (self.dt_val / 6) * (k1 + 2 * k2 + 2 * k3 + k4)
            # dynamic constraints
            self.g = ca.vertcat(self.g, state_next - state_next_RK4)
...
if __name__ == '__main__':
    ...
    # Find soluton time
    times, solution_list, obs_history = optimizer.solve_mpc(
        start, end, t0, mpc_iter, False)
    ...
    def init():
        ...
        lines = [ax2.plot(uav[0, 0:1], uav[1, 0:1])[0]
                 for uav in position_data]
